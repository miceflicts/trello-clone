...
function DarkModeSwitcher() {
  ...
  const matchThemeToUserPreference = () => {
      isDarkMode ? setCurrentTheme("dark") : setCurrentTheme("light");
  };

  const matchThemeToUserClick = () => {
  ...
  const handleClick = () => {
    matchThemeToUserClick();
  }

  useEffect(() => {
    matchThemeToUserPreference();
  }, []);
  ...
  return (
    ...
    <>
        <div>
            <div className="w-[35px] h-[35px] flex flex-col justify-center ml-3">
                <input type="checkbox" name="light-switch" className="light-switch sr-only" />
                <label onClick={handleClick} className="relative cursor-pointer p-2" htmlFor="light-switch">
                    ...
                    <svg className="hidden dark:block min-w-full min-h-full" width="16" height="16" xmlns="http://www.w3.org/2000/svg">
